...
class Enemy2(pygame.sprite.Sprite):
    def __init__(self, x, y):
        ...
        for animation in ['Idle', 'Run', 'Death']:
            ...
            for i in range(num_of_frames):
                img = pygame.image.load(f'img/enemy2/{animation}/{i}.png').convert_alpha()
                img = pygame.transform.scale(img, (80, 80))
                temp_list.append(img)
            self.animation_list.append(temp_list)
        self.image = self.animation_list[self.action][self.frame_index]
...
class FloatingText(pygame.sprite.Sprite):
    def __init__(self, x, y, text="Hit", color=YELLOW):
        super().__init__()
        self.image = small_font.render(text, True, color)
        self.rect = self.image.get_rect(center=(x, y))
        self.counter = 0

    def update(self, time_scale):
        self.rect.y -= 1.5 * time_scale
    ...
    def update(self):
        ...
        for enemy in enemy_group:
            if self.rect.colliderect(enemy.rect) and enemy.alive:
                enemy.health -= 50
                total_hits += 1
                floating_text_group.add(FloatingText(enemy.rect.centerx, enemy.rect.top - 10, "Hit"))
                self.kill()
                break
...
def show_result_screen():
    ...
    pygame.draw.rect(screen, (40, 40, 40), (170, 70, 460, 460), border_radius=15)
    ...
    draw_text(f"RANK {rank}", large_font, rank_color, WIDTH//2, 190, center=True)
    ...
    draw_text("Press R To Restart", small_font, GREEN, WIDTH//2, 485, center=True)
    draw_text("Press ESC To Quit", small_font, GRAY, WIDTH//2, 505, center=True)
...
def show_level_transition():
    ...
    pygame.draw.rect(screen, (40, 40, 40), (180, 120, 440, 320), border_radius=15)
    ...
    draw_text(f"Welcome To Level {level + 1}", font, WHITE, WIDTH//2, 230, center=True)
    draw_text("Difficulty Increased", small_font, RED, WIDTH//2, 290, center=True)
    ...
    draw_text("+ Enemy Bullet Speed", small_font, WHITE, WIDTH//2, 390, center=True)
    draw_text("Press ENTER To Continue", small_font, GREEN, WIDTH//2, 470, center=True)
...
while True:
    ...
    if game_state == "START":
        ...
        draw_text("Time Moves When You Move", small_font, YELLOW, WIDTH//2, 210, center=True)
        ...
        draw_text("START GAME", small_font, BLACK, start_btn_rect.centerx, start_btn_rect.centery - 10, center=True)

        draw_text("Click Button or Press ENTER", small_font, WHITE, WIDTH//2, 420, center=True)
        draw_text("WASD = Move", small_font, GREEN, WIDTH//2, 470, center=True)
        draw_text("Mouse Click / F = Shoot", small_font, GREEN, WIDTH//2, 500, center=True)
